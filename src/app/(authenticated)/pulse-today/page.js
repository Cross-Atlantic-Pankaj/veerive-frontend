...
export default function PulseToday() {
    ...
    try {
      ...
      if (pageNum === 1) {
        ...
        setDisplayItems((prevDisplayItems) => [
          ...prevDisplayItems,
          ...processNewContexts(newContexts),
        ]);
      }
  ...
  const processInitialContexts = (allContexts) => {
    const allTypeOneContexts = allContexts.filter((c) => c.containerType === 'Type-One');
    const allOtherContexts = allContexts.filter((c) => c.containerType !== 'Type-One');
    ...
    allTypeOneContexts.forEach((context) => {
      ...
      if (currentGroup.length === 3 || allTypeOneContexts.indexOf(context) === allTypeOneContexts.length - 1) {
        if (currentGroup.length > 0) {
          const maxDisplayOrder = Math.max(...currentGroup.map((c) => c.displayOrder || 0));
          typeOneGroups.push({ type: 'group', items: [...currentGroup], displayOrder: maxDisplayOrder });
          currentGroup = [];
        }
      }
    });
    ...
    return [...typeOneGroups, ...otherItems].sort((a, b) => b.displayOrder - a.displayOrder);
  };

  const processNewContexts = (newContexts) => {
    const newTypeOneContexts = newContexts.filter((c) => c.containerType === 'Type-One');
    const newOtherContexts = newContexts.filter((c) => c.containerType !== 'Type-One');
    ...
    newTypeOneContexts.forEach((context) => {
      ...
      if (currentGroup.length === 3 || newTypeOneContexts.indexOf(context) === newTypeOneContexts.length - 1) {
        if (currentGroup.length > 0) {
          const maxDisplayOrder = Math.max(...currentGroup.map((c) => c.displayOrder || 0));
          newTypeOneGroups.push({ type: 'group', items: [...currentGroup], displayOrder: maxDisplayOrder });
          currentGroup = [];
        }
      }
    });
    ...
    return [...newTypeOneGroups, ...newOtherItems].sort((a, b) => b.displayOrder - a.displayOrder);
  };
          ...
          >
            {displayItems.map((displayItem, index) => {
              ...
              if (displayItem.type === 'group') {
                return (
                  <div key={`group-${index}`} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 sm:mb-8">
                    {displayItem.items.map((context) => renderContextBox(context, isLastItem))}
                  </div>
                );
              } else {
                return (
