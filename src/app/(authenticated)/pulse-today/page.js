...
export default function PulseToday() {
  ...
  }, [checkRightSidebarHeight, contexts, sidebarMessage, trendingThemes, expertPosts]);
  ...
  const groupContexts = (allContexts) => {
    ...
    typeOrder.forEach(type => {
      ...
      grouped[type].forEach((context, index) => {
        ...
        if (currentRow.length === maxPerRow || index === grouped[type].length - 1) {
  ...
  const renderContextBox = (context, isLastItem) => {
    ...
    switch (context.containerType) {
            ...
            <div className="px-3 py-2 sm:px-4 sm:py-3">
              <div className="flex flex-wrap gap-1 sm:gap-2 mb-1">
                {[...context.sectors, ...context.subSectors].map((name, idx) => (
                  <span key={idx} className="text-[10px] sm:text-xs text-black-600 relative inline-block font-medium border-b-2 border-green-500">
                    {name}
                  </span>
                ))}
              </div>
              <h3 className="text-xs sm:text-sm font-semibold text-gray-900 leading-snug">{context.contextTitle}</h3>
            </div>
          </div>
        );
      ...
      case 'Type-Two':
        return (
          <div ref={isLastItem ? lastContextCallback : null} className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 flex flex-col p-4 sm:p-5">
      ...
      case 'Type-Five':
        return (
          <div ref={isLastItem ? lastContextCallback : null} className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 p-4 sm:p-6">
            <div className="text-black-600 text-[10px] sm:text-xs font-semibold mb-2">{context.contextTitle}</div>
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <div className="w-full sm:w-1/3">
                {context.bannerImage ? (
                  <img src={context.bannerImage} alt="Banner" className="w-full h-40 sm:h-48 object-cover rounded-lg" />
                ) : (
                  ...
                  <div className="flex flex-col gap-0">
                    <div className="flex flex-col sm:flex-row items-start">
                      <div className="flex-1 w-full sm:w-auto">
                        <div className="font-semibold text-gray-800 text-[10px] sm:text-xs hover:text-indigo-600 transition-colors leading-none">{context.posts[0].postTitle}</div>
                      </div>
                      {context.posts.slice(1, Math.ceil(context.posts.length / 2)).map((post, i) => (
                        <div key={i} className="flex-1 w-full sm:w-auto border-l-0 sm:border-l border-gray-100 pl-0 sm:pl-4 mt-2 sm:mt-0">
                          <div className="font-semibold text-gray-800 text-[10px] sm:text-xs hover:text-indigo-600 transition-colors leading-none">{post.postTitle}</div>
                        </div>
                      ))}
                    </div>
                    <div className="flex flex-col sm:flex-row items-start mt-3">
                      <div className="flex-1 w-full sm:w-auto">
                        {summaryPoints.length > 0 ? (
                          summaryPoints.map((point, i) => (
                            <div key={i} className="text-gray-600 text-xs sm:text-sm leading-tight mt-0">{point}</div>
                          ))
                        ) : (
                          <div className="text-gray-400 text-xs sm:text-sm italic leading-tight mt-0">Summary will be available soon</div>
                        )}
                      </div>
      ...
      case 'Type-Num':
        return (
          <div ref={isLastItem ? lastContextCallback : null} className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 p-4 sm:p-6 h-fit">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
              <div className="text-3xl sm:text-4xl font-bold text-indigo-600 whitespace-nowrap">{context.dataForTypeNum}</div>
              <div className="flex-1">
                {summaryPoints.length > 0 ? (
                  ...
                  <p className="text-gray-400 text-xs sm:text-sm italic">Summary will be soon</p>
                )}
              </div>
            </div>
          </div>
    ...
    <Suspense fallback={<div className="text-center py-10">Loading...</div>}>
      <main className="px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-8 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
        <div ref={containerRef} className="flex flex-col lg:flex-row gap-6 lg:gap-8 relative">
          <div 
            ref={mainContentRef}
            className={`w-full transition-all duration-300 ease-in-out ${expandLeft ? 'lg:w-full' : 'lg:w-[72%]'}`}
          >
            {groupedContexts.map((group, groupIndex) => {
              const isLastGroup = groupIndex === groupedContexts.length - 1;
              let cols;
              if (group[0]?.containerType === 'Type-One' || group[0]?.containerType === 'Type-Two') {
                cols = 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3';
              } else if (group[0]?.containerType === 'Type-Three' || group[0]?.containerType === 'Type-Four') {
              ...
              return (
                <div key={groupIndex} className={`grid ${cols} gap-4 sm:gap-6 mb-6 sm:mb-8`}>
                  {group.map((context, index) => {
                    const isLastItem = isLastGroup && index === group.length - 1 && hasMore;
                    return (
                      <div key={context.id}>
          ...
          {(sidebarMessage || trendingThemes.length > 0 || expertPosts.length > 0) && (
            <div ref={rightSidebarRef} className="w-full lg:w-[28%] flex flex-col gap-6">
              {sidebarMessage && (
                <div className="bg-white p-4 sm:p-5 rounded-xl shadow-md">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-teal-500 text-base sm:text-lg">✦</span>
                    <span className="font-semibold text-gray-900 text-base sm:text-lg">{sidebarMessage.title}</span>
                  </div>
                  <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">{sidebarMessage.content}</p>
                </div>
              )}
              ...
              {trendingThemes.length > 0 && (
                <div className="bg-white p-4 sm:p-5 rounded-xl shadow-md">
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="font-semibold text-base sm:text-lg text-gray-900">Trending Themes</h2>
                    <Link href="/trend-analyzer" className="text-indigo-600 text-xs sm:text-sm font-medium hover:underline">
                      VIEW ALL →
                    </Link>
                  </div>
                  <div className="space-y-4">
                    {trendingThemes.map((theme) => (
                      <div key={theme.id} className="border-b border-dashed border-gray-200 pb-4 last:border-0">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <h3 className="font-semibold text-xs sm:text-sm text-gray-900">{theme.title}</h3>
                            <p className="text-[10px] sm:text-xs text-gray-500 mt-1">{theme.sectors.join(' • ')}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div className="bg-white p-4 sm:p-5 rounded-xl shadow-md">
                  ...
                  <div className="space-y-4">
                    {expertPosts.map((post, index) => (
                      <div key={index} className="border-b border-dashed border-gray-200 pb-3 last:border-none">
                        <h3 className="text-xs sm:text-sm font-semibold text-gray-900 hover:text-indigo-600 transition-colors">{post.postTitle}</h3>
                      </div>
                    ))}
                  </div>
                </div>
