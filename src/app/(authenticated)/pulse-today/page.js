...
export default function PulseToday() {
  ...
  const formatSummary = (summary) => {
    ...
    for (let i = 0; i < points.length; i += groupSize) {
      const group = points.slice(i, i + groupSize).map((p) => p.trim()).join(' ');
    ...
    while (i < contexts.length) {
      ...
      if (type === 'Type-One' || type === 'Type-Two') {
        if (
          ...
          (currentGroup.every((c) => ['Type-One', 'Type-Two'].includes(c.containerType)) ||
        ...
        currentGroup = [];
        if (i + 1 < contexts.length && ['Type-Three', 'Type-Four'].includes(contexts[i + 1].containerType)) {
      ...
      case 'Type-Two':
        return (
          ...
          >
            ...
            <h2 className="text-base sm:text-lg font-bold text-gray-900 mb-3 line-clamp-2 leading-tight">{context.contextTitle}</h2>
            <div className="mb-4">
      ...
      case 'Type-Five':
        return (
          <div
            ...
            className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 p-4 sm:p-6"
          >
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 line-clamp-2 leading-tight">{context.contextTitle}</h2>
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
              <div className="w-full sm:w-1/3">
                {context.bannerImage ? (
                ...
                <div className="text-gray-600 mt-4 text-xs sm:text-sm line-clamp-3">{summaryPoints?.length > 0 ? summaryPoints.join(' ') : 'Summary will be available soon'}</div>
              </div>
              <div className="flex-1">
                {context.posts?.map((post, i) => (
                  <div key={i} className="border-t border-gray-100 pt-1 mt-1">
        ...
        return (
          ...
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
              <div className="text-3xl sm:text-4xl font-bold text-indigo-600 whitespace-nowrap">{context.dataForTypeNum || 'US$ XX Billion'}</div>
              <div className="flex-1">
                {summaryPoints?.length > 0 ? (
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          <div className="w-full lg:w-[72%]">
            {groupedContexts.map((group, groupIndex) => {
              ...
              const cols = group.every((c) => ['Type-One', 'Type-Two'].includes(c.containerType))
                ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'
                : group.every((c) => ['Type-Three', 'Type-Four'].includes(c.containerType))
                ...
                {trendingThemes.map((theme) => (
                  <div key={theme.id} className="border-b border-dashed border-gray-200 pb-4 last:border-0">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-xs sm:text-sm text-gray-900 line-clamp-2">{theme.title}</h3>
              ...
              <div className="space-y-4">
                {expertPosts.map((post, index) => (
                  <div key={index} className="border-b border-dashed border-gray-200 pb-3 last:border-none">
                    <h3 className="text-xs sm:text-sm font-semibold text-gray-900 line-clamp-2 hover:text-indigo-600 transition-colors">{post.postTitle}</h3>
                  </div>
                ))}
              </div>
              {hasMore && (
