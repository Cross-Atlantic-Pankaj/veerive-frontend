...
export default function SavedItems() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (hasNewItems) {
          ...
          // If no new items, check if previous items exist; if not, clear previousSavedItems
          const hasPrevItems = 
  ...
  const handleUnsaveContext = async (contextId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Refetch to check if all items are unsaved
  ...
  return (
    <div className="w-full">
      {/* Headline: "Saved Items" at the top center */}
      ...
      {/* Main Content */}
      {!hasPreviousItems ? (
        <div className="w-full mt-2 text-gray-600 text-center">
          No items Saved.
        </div>
      ) : (
        <div className="flex w-full mt-4">
          {/* Left Sidebar - Render only if there are saved items */}
