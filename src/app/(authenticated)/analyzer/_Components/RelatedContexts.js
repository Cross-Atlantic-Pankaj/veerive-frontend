...
const RelatedContexts = ({ contexts }) => {
  if (!contexts || contexts.length === 0) {
    return null;
  }

  const typeOneContexts = contexts.filter(
  ...
  return (
    <div className="mb-6 max-full mx-auto md:py-6 p-2 md:px-8">
      ...
      <div className="space-y-6">
        {otherContexts.map((relatedContext, index) => (
          <div key={`other-${relatedContext._id}-${index}`} className="grid grid-cols-1">
            {renderContainerType(relatedContext, `other-${relatedContext._id}-${index}`)}
          </div>
        ))}
        {groupedTypeOneContexts.map((group, groupIndex) => (
          ...
          >
            {group.map((relatedContext, index) => (
              renderContainerType(
                relatedContext,
                `type-one-${relatedContext._id}-${groupIndex}-${index}`
              )
            ))}
          </div>
        ))}
