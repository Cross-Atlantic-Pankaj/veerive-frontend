...
import TileTemplate from '@/models/TileTemplate'; // Corrected import
...
export async function GET(request) {
  try {
    ...
    let contexts = await Context.find().select('contextTitle _id').lean();
    ...
    if (normalizedSlug !== canonicalSlug) {
      return NextResponse.redirect(
        new URL(`/context-details/${canonicalSlug}`, request.url)
      );
    }
    ...
    const signalSubCategories = targetContext.signalSubCategories.map(
      (subSignal) => subSignal.subSignalName
    );
    ...
    if (targetContext.posts && targetContext.posts.length > 0) {
      const uniquePosts = Array.from(
        new Map(
          targetContext.posts.map((post) => [post.postId?._id.toString(), post])
        ).values()
    ...
    console.log('context Data:', JSON.stringify(contextPosts[0], null, 2));
